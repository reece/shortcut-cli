...
class Importer:
    ...
    def migrate_repo(self, repo_name):
        for issue in self.gz.fetch_issues(self.github_org, repo_name):
            ...
            if not is_epic:
                _logger.info(f"TESTING: {issue_abbr}: Skipping non-Epic")
                continue
            elif not issue.labels:
                _logger.info(f"TESTING: {issue_abbr}: Skipping issue without labels")
                continue

            if issue.html_url in self.migrated:
                _logger.info("Skipping %s; already migrated to %s" % (issue.html_url, self.migrated[issue.html_url]))
                continue
    ...
    def migrate_issue(self, issue):
        ...
        # todo: repo -> project, labels, closed_at?, user (creator?), closed_by?
        body = dict(
            name=issue.title,
